...
if view_option == "Students":
    ...
    st.dataframe(pd.read_sql_query(
        "SELECT enrollments.id, students.name AS student_name, students.roll_no, students.department AS student_department, "
        "courses.name AS course_name, courses.duration, enrollments.enrollment_date "
        "FROM enrollments "
